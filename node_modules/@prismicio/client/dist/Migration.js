...
import { filledContentRelationship, filledImage, filledLinkToMedia, rtImageNode } from "./lib/isValue.js";
...
var Migration = class {
	...
	*/
	createDocumentFromPrismic(document, title) {
		const doc = new PrismicMigrationDocument(this.#migratePrismicDocumentData({
			type: document.type,
			lang: document.lang,
			uid: document.uid,
			tags: document.tags,
	...
	*/
	#migratePrismicDocumentData(input) {
		...
		if (filledImage(input)) {
			...
			for (const name in thumbnails) if (filledImage(thumbnails[name])) image[name] = this.createAsset(thumbnails[name]);
			return image;
		}
